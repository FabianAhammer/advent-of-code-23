...
//
// int counter = 0;
// foreach (string gameLine in input.Split("\n"))
// {
//     Console.WriteLine(gameLine);
//
//     string gamecontent = gameLine.Split(":")[1];
//
//     //check above 14
//     if (new Regex("(1[5-9])|[2-9]\\d").Match(gamecontent).Success)
//     {
//         Console.WriteLine("Exiting, number above 15");
//         continue;
//     }
//
//     //check 14 not red
//     if (new Regex("14 green").Match(gamecontent).Success)
//     {
//         Console.WriteLine("Exiting,  14 green");
//         continue;
//     }
//
//     if (new Regex("1[3-4] red").Match(gamecontent).Success)
//     {
//         Console.WriteLine("Exiting,  13/14 red");
//         continue;
//     }
//
//     string trimmedNumber = gameLine.Split(":")[0].Replace("Game", " ").Trim();
//
//     Console.WriteLine("Game " + trimmedNumber + " is possible");
//     counter += Int32.Parse(trimmedNumber);
// }
//
...
const string input = @"Game 1: 18 red, 8 green, 7 blue; 15 red, 4 blue, 1 green; 2 green, 17 red, 6 blue; 5 green, 1 blue, 11 red; 18 red, 1 green, 14 blue; 8 blue
Game 2: 16 blue, 12 green, 3 red; 13 blue, 2 red, 8 green; 15 green, 3 red, 16 blue
Game 3: 6 green, 15 red; 1 green, 4 red, 7 blue; 9 blue, 7 red, 8 green
Game 4: 8 red, 2 blue; 11 red, 5 blue, 1 green; 12 red, 1 green, 5 blue; 1 blue; 2 blue, 9 red
Game 5: 9 blue, 3 red, 12 green; 3 green, 4 red, 17 blue; 15 blue, 2 green, 5 red; 3 blue, 5 green, 6 red; 6 red, 4 blue, 7 green; 3 green, 10 blue
...
foreach (var s in input.Split("\n"))
{
    ...
    string gameLine = s.Split(":")[1].Replace(";", ",");

    int maxRed = gameLine.Split(",").Where(e => e.Contains("red"))
        .Aggregate(0, (a, b) =>
        {
            int bInt = Int32.Parse(b.Split("red")[0].Trim());
            return bInt > a ? bInt : a;
        });
    int maxBlue = gameLine.Split(",").Where(e => e.Contains("blue"))
        .Aggregate(0, (a, b) =>
        {
            int bInt = Int32.Parse(b.Split("blue")[0].Trim());
            return bInt > a ? bInt : a;
        });

    int maxGreen = gameLine.Split(",").Where(e => e.Contains("green"))
        .Aggregate(0, (a, b) =>
        {
            int bInt = Int32.Parse(b.Split("green")[0].Trim());
            return bInt > a ? bInt : a;
        });
    sumOfNumber += maxRed * maxBlue * maxGreen;
}
