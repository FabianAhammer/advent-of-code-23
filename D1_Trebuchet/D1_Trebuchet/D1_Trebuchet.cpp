...
{
    const std::string str = "tmmnhlxzpj1eightldxhjnone97\\"
    ...
    for (unsigned long long i = 0; i < str.length(); ++i)
    {
        if (str[i] == '\\')
        {
            const int readValue = LineHarvester::advancedParseLineToInts(inputRead);
            std::cout << "Line of " + inputRead + " converts to " + std::to_string(readValue) + "\n";
            value += readValue;
            inputRead = "";
        }
        else
...
int main1(int argc, char* argv[])
{
    ...
    for (unsigned long long i = 0; i < str.length(); ++i)
    {
        if (str[i] == '\\')
        {
            value += LineHarvester::parseLineToInts(inputRead);
            inputRead = "";
        }
        else
        {
